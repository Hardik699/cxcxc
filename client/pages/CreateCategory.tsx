...
export default function CreateCategory() {
  ...
  const [favourites, setFavourites] = useState<Set<string>>(() => {
    try {
      const stored = localStorage.getItem("category_favourites");
  ...
  const toggleFavourite = (id: string, e: React.MouseEvent) => {
    ...
    setFavourites((prev) => {
      ...
      localStorage.setItem("category_favourites", JSON.stringify([...next]));
      return next;
    });
  };
  ...
  const getFilteredCategories = () => {
    ...
    return [...filtered].sort((a, b) => {
      const aFav = favourites.has(a._id) ? 0 : 1;
      const bFav = favourites.has(b._id) ? 0 : 1;
      return aFav - bFav;
    });
  };
  ...
  return (
    <Layout title="Categories">
                    ...
                    {paginatedCategories.map((category, idx) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          ...
                          >
                            <span className={`w-2 h-2 rounded-full ${(category.status || "active") === "active" ? "bg-emerald-600" : "bg-red-600"}`}></span>
                            {((category.status || "active").charAt(0).toUpperCase() + (category.status || "active").slice(1))}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <button
                              onClick={(e) => toggleFavourite(category._id, e)}
                              className={`inline-flex items-center justify-center p-2 rounded-lg transition-all active:scale-95 shadow-sm hover:shadow-md ${
                                favourites.has(category._id)
                                  ? "bg-yellow-400 text-white hover:bg-yellow-500"
                                  : "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-500 dark:text-yellow-400 hover:bg-yellow-400 hover:text-white"
                              }`}
                              title={favourites.has(category._id) ? "Remove from Favourites" : "Add to Favourites"}
                            >
                              <Star className={`w-4 h-4 ${favourites.has(category._id) ? "fill-current" : ""}`} />
                            </button>
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
      ...
      {/* Change Log Modal */}
      {logCategory && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setLogCategory(null)}>
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-lg w-full border border-gray-200 dark:border-slate-700 max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-5 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-slate-400 capitalize">{logCategory.name}</p>
                </div>
              </div>
              <button onClick={() => setLogCategory(null)} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 text-gray-500 dark:text-slate-400 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="px-5 py-3 bg-blue-50 dark:bg-blue-900/20 border-b border-blue-100 dark:border-blue-800 flex items-center gap-2 text-xs text-blue-700 dark:text-blue-300">
              ...
              <span>Created by <span className="font-semibold">{logCategory.createdBy || "admin"}</span></span>
              <span className="mx-1">·</span>
              <Clock className="w-3.5 h-3.5" />
              <span>{new Date(logCategory.createdAt).toLocaleString("en-IN", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}</span>
            </div>

            {/* Log entries */}
            <div className="overflow-y-auto flex-1 p-5">
              {!logCategory.editLog || logCategory.editLog.length === 0 ? (
                ...
                <div className="space-y-3">
                  {[...logCategory.editLog].reverse().map((entry, idx) => (
